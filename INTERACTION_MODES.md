...
- **操作**: 右クリック先に艦隊が移動
- **移動制限**: 80px未満の近距離移動を防止（誤操作回避）
- **エリア外操作**: ゲームエリア外での右クリック時に選択解除
...
selectedFleets.forEach(fleet => {
    if (fleet.interactionMode === 'move') {
        ...
        const distance = Math.sqrt(
            Math.pow(x - 20 - fleet.x, 2) + 
            Math.pow(y - 20 - fleet.y, 2)
        );
...
2. 右クリック → **距離チェック**（80px以上）、**ゲームエリア内確認**後、艦隊が**進行方向に回転してから移動開始**、**半透明ゴースト艦隊が移動先に表示**、**選択状態が自動解除**
3. ゲームエリア外で右クリック → **移動せずに選択解除**（UIパネル上での操作）
4. 同じ艦隊をダブルクリック → 赤色枠で回転モード
5. 右クリック → 艦隊がその方向に**滑らかに回転開始**し、**選択状態が自動解除**
6. 戦闘発生時 → 敵への応戦で**滑らかな自動向き変更**（現実的な戦闘動作）
