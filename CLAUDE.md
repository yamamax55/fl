...
## データベースシステム（新規追加）
### 階級管理システム
- **データベース**: `military_ranks.db` (SQLite)
- **テーブル**: `ranks`（階級ID、階級名、序列）
- **階級データ**: 日本陸軍19階級（元帥～二等兵）
- **管理スクリプト**: `database.js`

### 提督管理システム  
- **データベース**: `admiral_db.db` (SQLite)
- **テーブル**: 
  - `admiral_basic`（提督基本情報・階級参照）
  - `admiral_stats`（11種類の能力値、各0-120）
- **サンプル提督**: ヤン・ウェンリー、ラインハルト等
- **管理スクリプト**: `admiral_database.js`

### 艦隊管理システム
- **データベース**: `fleet_db.db` (SQLite)
- **テーブル**:
  - `fleets`（艦隊基本情報・陣営・戦力）
  - `fleet_command`（指揮系統・役職兼務制限）
- **指揮系統**: 司令官・副司令官・参謀（役職兼務禁止）
- **管理スクリプト**: `fleet_database.js`

### ゲーム内統合
- **DatabaseService**: ブラウザ環境用静的データサービス
- **艦隊クリック時表示**: 左側パネルに司令官情報（階級・氏名・年齢）を表示
- **指揮系統可視化**: 役職別の明確な表示

### データベース実行コマンド
- 階級DB作成・確認: `node database.js`
- 提督DB作成・ヤン情報表示: `node admiral_database.js`
- 艦隊DB作成・司令官情報表示: `node fleet_database.js`
